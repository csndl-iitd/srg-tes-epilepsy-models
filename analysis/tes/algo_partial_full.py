...
class AlgoParFull:
    ...
    def compute_threshold(self,df):
        ...
        while i < df.shape[1]:
            ...
            covr = self.compute_coverage(df[i],start_stop['start_points'],start_stop['stop_points'])
            for j in covr:
                coverage.append(j)
            i+=1
    ...
    def compute_param_df(self,df):
        ...
        while n < df.shape[1]:
            ...
            if not k['start_points']:
                ...
                itr.append(n)
                trans_time=algo.compute_transition_time(df[n],k['start_points'].copy())
                fp_time=self.compute_fpt(k['start_points'].copy(),k['stop_points'].copy())
                sync_time = self.compute_sync(k['start_points'].copy(),k['stop_points'].copy())
                cov=self.compute_coverage(df[n],k['start_points'].copy(),k['stop_points'].copy())
