...
class UmapAlgo:
    ...
    def compute_start_stop(self, df, embed):
        ...
        while i + 4 < len(embed):
            ...
            if (embed[i + 4] - start_dum) == 400 and check_go(i) == True:
                ...
                i += 1
        return {"start_points": start, "stop_points": stop}
    ...
    def compute_param_df(self, data):
        ...
        while c < data.shape[1]:
            ...
            coverage.append(
                self.compute_coverage(data[c], l["start_points"], l["stop_points"])
            )
            c += 1
        ...
        param_dict = dict(
            zip(
                ["tran_per", "sync", "trans", "fpt", "cov"],
                [tran_per, sync, tran_time, fpt, cov],
            )
        )
        return param_dict
