...
class AnalysisFunc:
    ...
    def node_count(self,df):
        ...
        Returns:
            dictionary: the keys are ['node_count','timestep','node_list']
        """
        c=1
        n=0
        # to iterate over rows
        i=0
        # to store nodes which enters the main cluster
        node_list=[]
        # to store timesteps at which either node or number of node enetring main cluster changes
        timestep=[]
        # to store node count
        node_count=[]
        # empty list to compare node lists at every timestep
        compare=[]

        #main cluster computation
        mc=self.compute_main_cluster(df)
        while i <5000:
            j=df.iloc[i,:]
            p=j[j==mc]
            if len(p.index!=0):
                if list(p.index) != compare:
                    # print(p)
                    # print('good')
                    #print(f'Start time for main cluster is {i} for {c}th time with total nodes {len(p.index)} node {list(p.index)} enters')
                    # print(f'Start time for main cluster is {i} for {c}th time with total nodes {len(p.index)}')
                    node_list.append(list(p.index))
                    timestep.append(i)
                    node_count.append(len(p.index))
                    c+=1
                    compare=list(p.index)
                else:
                    pass
        ...
        keys=['node_count','timestep','node_list']
        values=[node_count,timestep,node_list]
